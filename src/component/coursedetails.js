...
export default function CourseDetail() {
  ...
  useEffect(() => {
    if (checkingAuth) return; // wait for auth check
    if (student && location.state?.openRegister) {
      setOpenRegisterModal(true);
      navigate(location.pathname, { replace: true }); // remove state after use
    }
  }, [checkingAuth, student, location.state, navigate]);
  ...
  const handleRegisterClick = () => {
    if (checkingAuth) return; // wait for auth
  ...
  const handleSubmitRegistration = async () => {
    ...
    try {
      ...
      const response = await fetch("http://localhost:7000/api/register-course", {
        ...
        body: JSON.stringify({ courseId: id, paymentMethod }),
      });
    ...
    <>
      <Box sx={{ minHeight: "100vh", backgroundColor: "#f1f5f9", py: 10 }}>
        <Container maxWidth="md">
          <Paper
            elevation={0}
            sx={{ p: 7, borderRadius: 4, backgroundColor: "#ffffff", border: "1px solid #e2e8f0" }}
          >
            {/* IMAGE */}
            <Box
              component="img"
